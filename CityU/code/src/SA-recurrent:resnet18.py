...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # 求注意力c_1
        h_1 = torch.tanh(self.unit(torch.cat(((x_1,c_0)))))
        c_1 = h_1

        # 求注意力c_2 
        h_2 = torch.tanh(self.unit(torch.cat((x_2,c_1))))
        a_2_1 = self.v(torch.tanh(self.attention(torch.cat((h_1,h_2)))))
        a_2_2 = self.v(torch.tanh(self.attention(torch.cat((h_2,h_2)))))
        a_2_1,a_2_2 = torch.softmax(torch.cat((a_2_1,a_2_2)),dim=0)
        c_2 = a_2_1*h_1+a_2_2*h_2

        # 求注意力c_3
        h_3 = torch.tanh(self.unit(torch.cat((x_3,c_2))))
        a_3_1 = self.v(torch.tanh(self.attention(torch.cat((h_1,h_3)))))
        a_3_2 = self.v(torch.tanh(self.attention(torch.cat((h_2,h_3)))))
        a_3_3 = self.v(torch.tanh(self.attention(torch.cat((h_3,h_3)))))
        a_3_1,a_3_2,a_3_3 = torch.softmax(torch.cat((a_3_1,a_3_2,a_3_3)),dim=0)
        c_3 = a_3_1*h_1+a_3_2*h_2+a_3_3*h_3
...
# 保存最佳模型参数
torch.save(best_model_wts_1, "../model/sa-recurrent-1.pth")
torch.save(best_model_wts_2, "../model/sa-recurrent-2.pth")
torch.save(best_model_wts_3, "../model/sa-recurrent-3.pth")
torch.save(best_model_wts_attention, "../model/sa-recurrent-attention.pth")
